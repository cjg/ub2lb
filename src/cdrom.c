		      ...
		      uint32_t length, char *dest)
{
    ...
    if (offset)
    {
        ...
        {
            if (!boot->phys->block_read(boot->phys->dev, block, 1, boot->tmpbuf))
                return -1;
    ...
    if (length / 2048)
    {
        ...
        if (!boot->phys->block_read(boot->phys->dev, block, blocks_read, dest))
            return -1;
...
static int iterate_entries(cdrom_boot_dev_t *dev, uint32_t block, uint32_t offset, uint32_t size, int (*iter_func)(struct iso9660_susp_entry *, void *userdata), void *userdata)
{
    ...
    do {
        ...
        {
            struct iso9660_susp_ce *ce = (struct iso9660_susp_ce *)e;
            block = ce->blk_be;
            offset = ce->off_be;
            size = ce->len_be;
...
static struct iso9660_susp_entry *get_entry(cdrom_boot_dev_t *dev, uint32_t block, uint32_t offset, uint32_t size, uint8_t sig[2])
{
    struct tmp_data d;
    context_t *ctx = dev->ctx;
    ...
    iterate_entries(dev, block, offset, size, my_iteration, &d);
    
    return d.entry;
}

static int fs_cdrom_find_file(cdrom_boot_dev_t *this, char *filename, uint32_t *fblock, uint32_t *fsize)
{
    context_t *ctx = this->ctx;
    char *local_filename;
    ...
    for (j = 0; j < depth; j++)
    {
        ...
        {
            if (cdrom_read(this, block, offset, sizeof(dirent), &dirent))
                return 0;
...

            {
                ...
                if (this->rr)
                {
                    struct iso9660_susp_entry *entry = get_entry(this, block, sua_off, sua_size, "NM");
                    if (entry)
                    {
                        ...
                        else 
                        {
                            if (entry->len > 5)
                            {
                                int __size = entry->len - 5;
                                fname = malloc(__size + 1);
                                if (!fname)
                                {
                                    return 0;
                                }
                                fname_alloc = 1;
                                memcpy(fname, &entry->data[1], __size);
                                fname[__size] = '\0';
                            }
                        }
                        free(entry);
                ...
                if (!fname)
                {
                    ...
                    if (cdrom_read(this, block, nameoffset, dirent.namelen, name))
                        return 0;
                ...
                if (!strcasecmp(elements[j], fname))
                {
                    ...
                    if ((j+1 < depth && type == 2) || (j+1 == depth && type != 2))
                    {
                        found = 1;
                    }
                }
...
static int fs_cdrom_load_file(cdrom_boot_dev_t *this, char *filename, void *buffer)
{
    uint32_t block;
    uint32_t size = 0;
    context_t *ctx = this->ctx;
...
boot_dev_t *cdrom_create()
{
    ...
    if (boot)
    {
        ...
        boot->phys = dev;
        boot->dev.load_file = fs_cdrom_load_file;
        boot->dev.destroy = fs_cdrom_destroy;
        boot->rr = 0;
        
        if (cdrom_read(boot, 16, 0, sizeof(boot->voldesc), &boot->voldesc))
        {
                goto fail;
        }
...
//        memcpy(&boot->voldesc, boot->tmpbuf, sizeof(boot->voldesc));
        ...
        if (strncmp(boot->voldesc.voldesc.magic, "CD001", 5))
        {
            goto fail;
        }
        ...
        /* Read the system use area and test it to see if SUSP is
           supported.  */
        if (cdrom_read(boot, boot->voldesc.rootdir.first_sector_be, 0, sizeof(boot->rootdir), &boot->rootdir))
        {
                     goto fail;
        }
...
//        memcpy(sua, &boot->tmpbuf[sua_pos], sua_size);
        cdrom_read(boot, boot->voldesc.rootdir.first_sector_be, sua_pos, sua_size, sua);
        ...
        if (entry->sig[0] == 'S' && entry->sig[1] == 'P')
        {
            ...
            entry = get_entry(boot, boot->voldesc.rootdir.first_sector_be, sua_pos, sua_size, "RR");
            if (entry)
            {
                boot->rr = 1;
                free(entry);
