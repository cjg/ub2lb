...
static int load_hunk(void *file, struct sheader *sh)
{
    ...
    {
        bzero(ptr, sh->size);
        bss_tracker->addr = (void*)((unsigned long)ptr + KERNEL_VIRT_BASE 
				    - KERNEL_PHYS_BASE);
		    ...
		    uint32_t virt)
{
    ...
    for (i=0; i<numrel; i++, rel++)
    {
        ...
        {
            case SHN_UNDEF:
		    printf("[ELF Loader] Undefined symbol '%s' in section '%s'\n",
                      (char *)((uint32_t)sh[shsymtab->link].addr) + sym->name,
                      (char *)((uint32_t)sh[eh->shstrndx].addr) + toreloc->name);
                return 0;

            case SHN_COMMON:
                printf("[ELF Loader] COMMON symbol '%s' in section '%s'\n",
                      (char *)((uint32_t)sh[shsymtab->link].addr) + sym->name,
                      (char *)((uint32_t)sh[eh->shstrndx].addr) + toreloc->name);
    ...
    for (i=0; i < eh->shnum; i++)
    {
        ...
        else if (sh[i].flags & SHF_ALLOC)
        {
            ...
            else
            {
                if (sh[i].size && !shown)
                {
                    printf("[BOOT] ELF: section loaded at %p (Virtual addr: %p)\n", sh[i].addr, sh[i].addr +
